...
/*
`ifdef ddr3_ip_inst_den1024Mb
    `include "1024Mb_ddr3_parameters.vh"
`elsif ddr3_ip_inst_den2048Mb
    `include "2048Mb_ddr3_parameters.vh"
`elsif ddr3_ip_inst_den4096Mb
...
*/
    ...
    // text macros
    `define ddr3_ip_inst_DQ_PER_DQS DQ_BITS/DQS_BITS
    `define ddr3_ip_inst_BANKS (1<<BA_BITS)
    `define ddr3_ip_inst_MAX_BITS (BA_BITS+ROW_BITS+COL_BITS-BL_BITS)
    `define ddr3_ip_inst_MAX_SIZE (1<<(BA_BITS+ROW_BITS+COL_BITS-BL_BITS))
    `define ddr3_ip_inst_MEM_SIZE (1<<MEM_BITS)
    `define ddr3_ip_inst_MAX_PIPE 4*CL_MAX
    ...
    // Mode Registers
    reg     [ADDR_BITS-1:0] mode_reg [`ddr3_ip_inst_BANKS-1:0];
    reg     burst_order;
    ...
    // command state
    reg     [`ddr3_ip_inst_BANKS-1:0] active_bank;
    reg     [`ddr3_ip_inst_BANKS-1:0] auto_precharge_bank;
    reg     [`ddr3_ip_inst_BANKS-1:0] write_precharge_bank;
    reg     [`ddr3_ip_inst_BANKS-1:0] read_precharge_bank;
    reg     [ROW_BITS-1:0] active_row [`ddr3_ip_inst_BANKS-1:0];
    reg     in_power_down;
    reg     in_self_refresh;
    ...
    integer ck_cke_cmd;
    integer ck_bank_write     [`ddr3_ip_inst_BANKS-1:0];
    integer ck_bank_read      [`ddr3_ip_inst_BANKS-1:0];
    ...
    time    tm_cke_cmd;
    time    tm_ttsinit;
    time    tm_bank_precharge [`ddr3_ip_inst_BANKS-1:0];
    time    tm_bank_activate  [`ddr3_ip_inst_BANKS-1:0];
    time    tm_bank_write_end [`ddr3_ip_inst_BANKS-1:0];
    time    tm_bank_read_end  [`ddr3_ip_inst_BANKS-1:0];
    time    tm_group_activate  [1:0];
    time    tm_group_write_end [1:0];

    // pipelines
    reg     [`ddr3_ip_inst_MAX_PIPE:0]  al_pipeline;
    reg     [`ddr3_ip_inst_MAX_PIPE:0]  wr_pipeline;
    reg     [`ddr3_ip_inst_MAX_PIPE:0]  rd_pipeline;
    reg     [`ddr3_ip_inst_MAX_PIPE:0]  odt_pipeline;
    reg     [`ddr3_ip_inst_MAX_PIPE:0]  dyn_odt_pipeline;
    reg     [BL_BITS:0]    bl_pipeline  [`ddr3_ip_inst_MAX_PIPE:0];
    reg     [BA_BITS-1:0]  ba_pipeline  [`ddr3_ip_inst_MAX_PIPE:0];
    reg     [ROW_BITS-1:0] row_pipeline [`ddr3_ip_inst_MAX_PIPE:0];
    reg     [COL_BITS-1:0] col_pipeline [`ddr3_ip_inst_MAX_PIPE:0];
    reg     prev_cke;
    ...
    // Memory Storage
`ifdef ddr3_ip_inst_MAX_MEM
    parameter RFF_BITS = DQ_BITS*BL_MAX;
     // %z format uses 8 bytes for every 32 bits or less.
    parameter RFF_CHUNK = 8 * (RFF_BITS/32 + (RFF_BITS%32 ? 1 : 0));
    reg [1024:1] tmp_model_dir;
    integer memfd[`ddr3_ip_inst_BANKS-1:0];
    ...
    begin : file_io_open
        ...
        for (bank = 0; bank < `ddr3_ip_inst_BANKS; bank = bank + 1)
            memfd[bank] = open_bank_file(bank);
    end
`else
    reg     [BL_MAX*DQ_BITS-1:0] memory  [0:`ddr3_ip_inst_MEM_SIZE-1];
    reg     [`ddr3_ip_inst_MAX_BITS-1:0]      address [0:`ddr3_ip_inst_MEM_SIZE-1];
...
`ifdef ddr3_ip_inst_MAX_MEM
    ...
    function integer_ddr3_ip_inst open_bank_file( input integer bank );
        integer fd;
        reg [2048:1] filename;
        begin 
    ...
    task_ddr3_ip_inst write_to_file( 
        input integer fd, 
        input integer index, 
...
`else
    function get_index;
        input [`ddr3_ip_inst_MAX_BITS-1:0] addr;
        begin : index
            get_index = 0;
...
`endif
    ...
    task memory_write;
        ...
        reg    [`ddr3_ip_inst_MAX_BITS-1:0] addr;
        begin
`ifdef ddr3_ip_inst_MAX_MEM
            addr = {row, col}/BL_MAX;
...
`else
            ...
            end else if (memory_used == `ddr3_ip_inst_MEM_SIZE) begin
...
`endif
    ...
    task erase_banks;
        input  [`ddr3_ip_inst_BANKS-1:0] banks; //one select bit per bank
        reg [BA_BITS-1:0] ba;
        reg [`ddr3_ip_inst_MAX_BITS-1:0] i;
        integer bank;
...
//        $display ("truebl4 = %d, relationship = %d, fromcmd = %h, cmd = %h", truebl4, relationship, fromcmd, cmd);
    ...
    task cmd_task;
        ...
        input [BA_BITS-1:0] bank;
        input [ADDR_BITS-1:0] addr;
        reg [`ddr3_ip_inst_BANKS:0] i;
        integer j;
        reg [`ddr3_ip_inst_BANKS:0] tfaw_cntr;
                    ...
                    PRECHARGE : begin
                        ...
                        if (|active_bank) begin
                            ...
                            if (mpr_en) begin
                                ...
                                for (i=0; i<`ddr3_ip_inst_BANKS; i=i+1) begin
    ...
    task data_task;
        ...
        begin
            ...
            if (dq_in_valid) begin // write data
                ...
                if (diff_ck) begin
                    for (i=0; i<DM_BITS; i=i+1) begin
                        bit_mask = bit_mask | ({`ddr3_ip_inst_DQ_PER_DQS{~dm_in_neg[i]}}<<(burst_position*DQ_BITS + i*`ddr3_ip_inst_DQ_PER_DQS));
                    end
                    memory_data = (dq_in_neg<<(burst_position*DQ_BITS) & bit_mask) | (memory_data & ~bit_mask);
                end else begin
                    for (i=0; i<DM_BITS; i=i+1) begin
                        bit_mask = bit_mask | ({`ddr3_ip_inst_DQ_PER_DQS{~dm_in_pos[i]}}<<(burst_position*DQ_BITS + i*`ddr3_ip_inst_DQ_PER_DQS));
                    end
            ...
            if (rdq_cntr > 0) begin // read data
                if (mpr_en) begin
`ifdef ddr3_ip_inst_MPR_DQ0 // DQ0 output MPR data, other DQ low				   
                    if (mpr_select == 2'b00) begin // Calibration Pattern
                        dq_temp = {DQS_BITS{{`ddr3_ip_inst_DQ_PER_DQS-1{1'b0}}, calibration_pattern[burst_position]}};
                    end else if (odts_readout && (mpr_select == 2'b11)) begin // Temp_ddr3_ip_inst Sensor (ODTS)
                        dq_temp = {DQS_BITS{{`ddr3_ip_inst_DQ_PER_DQS-1{1'b0}}, temp_sensor[burst_position]}};
                    end else begin // Reserved
                        dq_temp = {DQS_BITS{{`ddr3_ip_inst_DQ_PER_DQS-1{1'b0}}, 1'bx}};
                    end
`else // all DQ output MPR data
                    if (mpr_select == 2'b00) begin // Calibration Pattern
                        dq_temp = {DQS_BITS{{`ddr3_ip_inst_DQ_PER_DQS{calibration_pattern[burst_position]}}}};
                    end else if (odts_readout && (mpr_select == 2'b11)) begin // Temp_ddr3_ip_inst Sensor (ODTS)
                        dq_temp = {DQS_BITS{{`ddr3_ip_inst_DQ_PER_DQS{temp_sensor[burst_position]}}}};
                    end else begin // Reserved
                        dq_temp = {DQS_BITS{{`ddr3_ip_inst_DQ_PER_DQS{1'bx}}}};
                    end
`endif				   
            ...
            if (RANDOM_OUT_DELAY && (dqs_out_en || (|dqs_out_en_dly) || dq_out_en || (|dq_out_en_dly))) begin
                for (i=0; i<DQS_BITS; i=i+1) begin
                    ...
                    if (!write_levelization) begin
                        for (j=0; j<`ddr3_ip_inst_DQ_PER_DQS; j=j+1) begin
                            dq_out_en_dly[i*`ddr3_ip_inst_DQ_PER_DQS + j] <= #(tck_avg/2) dq_out_en;
                            if (dqsq_min == dqsq_max) begin
                                dq_out_dly   [i*`ddr3_ip_inst_DQ_PER_DQS + j] <= #(tck_avg/2 + dqsq_min) dq_out[i*`ddr3_ip_inst_DQ_PER_DQS + j];
                            end else begin
                                dq_out_dly   [i*`ddr3_ip_inst_DQ_PER_DQS + j] <= #(tck_avg/2 + $dist_uniform(seed, dqsq_min, dqsq_max)) dq_out[i*`ddr3_ip_inst_DQ_PER_DQS + j];
                            end
                        end
                    end
                end
...
`endif
    ...
    always @(negedge rst_n_in or posedge diff_ck or negedge diff_ck) begin : main
        ...
        if (!rst_n_in) begin
            ...
            if (|wr_pipeline || |rd_pipeline || |al_pipeline) begin
                ...
                for (i=0; i<`ddr3_ip_inst_MAX_PIPE; i=i+1) begin
    ...
    task dqs_even_receiver;
        ...
        reg [127:0] bit_mask;
        begin
            bit_mask = {`ddr3_ip_inst_DQ_PER_DQS{1'b1}}<<(i*`ddr3_ip_inst_DQ_PER_DQS);
            if (dqs_even[i]) begin
    ...
    begin
        if (dqs_in_valid) begin
            if ($time - tm_dqs[i/`ddr3_ip_inst_DQ_PER_DQS] < TDH) 
                $display ("%m: at time %t ERROR:   tDH violation on DQ bit %d by %t", $time, i, tm_dqs[i/`ddr3_ip_inst_DQ_PER_DQS] + TDH - $time);
    ...
    begin
        if (write_levelization && i<32) begin
            ...
            dq_out_en_dly[i*`ddr3_ip_inst_DQ_PER_DQS] <= #(TWLO) 1'b1;
            dq_out_dly[i*`ddr3_ip_inst_DQ_PER_DQS] <= #(TWLO) diff_ck;
`ifdef ddr3_ip_inst_WL_ALLDQ
            for (j=1; j<`ddr3_ip_inst_DQ_PER_DQS; j=j+1) begin
                dq_out_en_dly[i*`ddr3_ip_inst_DQ_PER_DQS+j] <= #(TWLO) 1'b1;
                dq_out_dly[i*`ddr3_ip_inst_DQ_PER_DQS+j] <= #(TWLO) diff_ck;
            end
`else
            for (j=1; j<`ddr3_ip_inst_DQ_PER_DQS; j=j+1) begin
                dq_out_en_dly[i*`ddr3_ip_inst_DQ_PER_DQS+j] <= #(TWLO + TWLOE) 1'b1;
                dq_out_dly[i*`ddr3_ip_inst_DQ_PER_DQS+j] <= #(TWLO + TWLOE) 1'b0;
            end
`endif
        end
        if (dqs_in_valid && ((wdqs_pos_cntr[i] < wr_burst_length/2) || b2b_write)) begin
            if (dqs_in[i] ^ prev_dqs_in[i]) begin
                ...
                if (!dq_out_en) begin
                    for (j=0; j<`ddr3_ip_inst_DQ_PER_DQS; j=j+1) begin
                        if ($time - tm_dq[(i%32)*`ddr3_ip_inst_DQ_PER_DQS+j] < TDS) 
                            $display ("%m: at time %t ERROR: tDS violation on DQ bit %d by %t", $time, i*`ddr3_ip_inst_DQ_PER_DQS+j, tm_dq[(i%32)*`ddr3_ip_inst_DQ_PER_DQS+j] + TDS - $time);
                        check_dq_tdipw[(i%32)*`ddr3_ip_inst_DQ_PER_DQS+j] <= 1'b1;
                    end
                end
